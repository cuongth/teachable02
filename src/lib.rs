...
fn process_instruction(
    ...
    let syspro_account = next_account_info(accounts_iter)?;

    if !system_program::check_id(syspro_account.key) {
    ...
    invoke_signed(
        ...
        &[user.clone(), user_derived_account.clone(), syspro_account.clone()],
        &[&[b"vault", user.key.as_ref(), &[bump_seed]]],
    )?;

    let mut account_data: UserStake =
        UserStake::try_from_slice(&user_derived_account.data.borrow()).unwrap();
